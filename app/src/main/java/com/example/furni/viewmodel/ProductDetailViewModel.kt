...
class ProductDetailViewModel @Inject constructor(
    ...
    private val _productDetail = MutableStateFlow(Product(isLoading = true))
    val productDetail: StateFlow<Product> = _productDetail

    private val _orderStatus = MutableStateFlow(OrderStatus(isLoading = true))
    val orderStatus: StateFlow<OrderStatus> = _orderStatus

    fun getProductDetail(productUID: String) = viewModelScope.launch {
        productDetailRepository.getProductDetail(productUID).collect {
            when (it) {
                is Resource.Loading -> {
    ...
    fun addToCart(userID: String, productID: String, product: Product, amount: Int) =
        viewModelScope.launch {
            productDetailRepository.addToCart(userID, productID, product, amount).collect {
                when (it) {
                    is Resource.Loading -> {
